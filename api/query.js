 ...
 * Types: specs | bottlenecks | kb | tasks | projects | search
 */
...
export default async function handler(req, res) {
  ...
  if (!state) return res.status(200).json({ ok: true, results: [], _note: 'No state saved yet.' });
  ...
  switch (type) {
    ...
    case 'projects': {
      ...
      const enriched = projects.map(p => {
        ...
        return { ...p, taskCount: tasks.length, completedTasks: tasks.filter(t => t.status === 'done').length, kbCount: kb.length, specCount: specs.length };
      });
